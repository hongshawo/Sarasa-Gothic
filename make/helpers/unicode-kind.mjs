...
export const isLongDash = (c, isTerm) => (isTerm ? c === 0x2e3a || c === 0x2e3b : false);
export const filterUnicodeRange = (a, fn) => {
	for (const c in a.cmap) {
		if (!fn(c - 0)) a.cmap[c] = null;
	}
	if (a.cmap_uvs) {
		for (const c in a.cmap_uvs) {
			const [su, _ss] = c.split(" ");
